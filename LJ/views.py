...
@app.route('/submit3d',methods = ['GET','POST'])
def submit3d():
    print(request.form.get("springDisplay"))
    run_params = {
        "time_end" : 3000 if request.form['time_end']=='' else int(request.form['time_end']),
    ...
    run_params["rc"] = run_params['r0'] * 1.2
    return run3d(run_params)

def run3d(run_params):
    # x = runner.main(run_params)
    # runner.main(run_params)
    return Response(stream_template("index3d.html",data = runner.main(run_params),iter_inc = run_params["print_every"],v_spring=run_params["spring_viz"]))
